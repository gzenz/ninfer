...
class Monitor:
    def __init__(self, cfg: Config) -> None:
        ...
        # The speculative backend in use (last seen in a request log record),
        # e.g. "dflash2". The dashboard labels the accept-rate tile/chart/log
        # with this instead of hard-coding "MTP".
    ...
    def _record_done(self, rec: dict, ts) -> None:
        ...
        # began. V3 emits it in the request log's engine_timing block (seconds);
        # fallback to the serve-log done line parsed in handle_serve_line.
        queue = rec.get("engine_timing", {}).get("queue_wait_seconds")
        if queue is None:
            rid = req.get("request_id")
            if rid is not None:
                queue = self._serve_queue.pop(rid, None)
        # Track the speculative backend (e.g. "dflash2") so the dashboard can
        # label the accept-rate tile/chart/log dynamically.
    ...
    def handle_serve_line(self, line: str) -> None:
        ...
        # Keep the speculative-backend accept summary (V3 done lines carry
        # "<backend> accepted X/Y") for the activity log.
        if " accepted" in line:
            self.kv_log.append({"t": int(time.time() * 1000), "line": line.strip()})
        # Parse done lines for queue time ("queue NNN ms" or "queue N.Ns"), keyed
        # by request id for _record_done to consume as a fallback when the JSONL
        # record lacks it.
        m_done = re.search(r"req#(\d+) done.*?queue\s+(\d+(?:\.\d+)?)\s*(ms|s)", line)
        if m_done:
            rid = int(m_done.group(1))
            seconds = float(m_done.group(2)) / (1000.0 if m_done.group(3) == "ms" else 1.0)
            self._serve_queue[rid] = seconds
