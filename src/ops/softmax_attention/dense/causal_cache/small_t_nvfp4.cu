// ninfer::ops::detail - NVFP4 (E2M1 + E4M3 group-16) split-KV small-T launch ownership.
...
void launch_nvfp4_partial(const Tensor& q, CacheInput input, const Tensor& positions,
    ...
    // Arena: K codes (Bc * D/2) + V codes (Bc * D/2) + V dequant BF16 (Bc * D * 2)
    //       = Bc * (D/2 + D/2 + D*2) = Bc * 3D
    ...
    const auto launch = [&]() {
        auto kernel = causal_attention_small_t_nvfp4_tiled_kernel<
            Geometry, TokenTile, Warps, MinBlocks, KeyBlock, true, MultiBatch, Masked,
            KernelInput>;
...
void launch_nvfp4_reduce(const Tensor& positions, const CausalSmallTInvocation& invocation,
                         std::int32_t splits, const Tensor& partial_acc,
                         const Tensor& partial_m, const Tensor& partial_l, Tensor& out,
                         cudaStream_t stream) {
    ...
    const auto launch = [&]<bool Offset>() {
        causal_attention_small_t_nvfp4_reduce_output_kernel<Geometry, DChunk, MultiBatch, Masked,
