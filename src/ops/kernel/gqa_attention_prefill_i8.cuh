...
// GQA KV-sharing variant: one CTA per KV head, Br=16 to fit all GroupSize Q heads.
inline constexpr int kGqaPrefillI8GqaBr = 16;
inline constexpr int kGqaPrefillI8GqaBc = 32;
...
// GQA-sharing SMEM layout (template on GroupSize G):
//   Q[G*Br*D] | Q_scale[G*Br*Groups*4] | K[Bc*D] | V[Bc*D] | V_f16[Bc*D*2]
//   | P[G*Br*Bc*2] | K_scale+V_scale[2*Bc*Groups*2]
//   | alpha[G*Br*4] | final_l[G*Br*4]
// K/V/V_f16 use the GQA Bc (32), not the original Bc (64).
template <int G>
inline constexpr int kGqaPrefillI8GqaKBytes = kGqaPrefillI8GqaBc * kGqaPrefillHeadDim;
template <int G>
inline constexpr int kGqaPrefillI8GqaVBytes = kGqaPrefillI8GqaBc * kGqaPrefillHeadDim;
template <int G>
inline constexpr int kGqaPrefillI8GqaVStageBytes =
    kGqaPrefillI8GqaBc * kGqaPrefillHeadDim * static_cast<int>(sizeof(__half));
template <int G>
inline constexpr int kGqaPrefillI8GqaScaleBytes =
    2 * kGqaPrefillI8GqaBc * kGqaPrefillI8Groups * static_cast<int>(sizeof(__half));

template <int G>
inline constexpr int kGqaPrefillI8GqaSmemBytes =
    G * kGqaPrefillI8GqaBr * kGqaPrefillHeadDim +                                    // Q (i8)
    G * kGqaPrefillI8GqaBr * kGqaPrefillI8Groups * static_cast<int>(sizeof(float)) + // Q_scale
    kGqaPrefillI8GqaKBytes<G> +                                                      // K (i8)
    kGqaPrefillI8GqaVBytes<G> +                                                      // V (i8)
    kGqaPrefillI8GqaVStageBytes<G> +                                                 // V_f16
    G * kGqaPrefillI8GqaBr * kGqaPrefillI8GqaBc * static_cast<int>(sizeof(__half)) + // P
    kGqaPrefillI8GqaScaleBytes<G> +                                                  // K+V scale
    2 * G * kGqaPrefillI8GqaBr * static_cast<int>(sizeof(float));                    // alpha + final_l
...
// ===========================================================================
// PackGQA variant (vLLM/FlashAttention approach): packs Q rows as
//   packed = token * G + qh_in_group
// so that Br rows in a tile span multiple Q heads within the same KV head
// group. K/V is loaded once into SMEM and all G Q heads compute against the
// same SMEM K/V, eliminating the GroupSize-x DRAM traffic amplification.
//
// Grid: (div_up(width*G, Br), KVHeads, 1). For GQA27 (G=6, KVHeads=4):
// div_up(1024*6,64)*4 = 96*4 = 384 CTAs, same as original 16*24=384, but K/V
// loaded 1x per kv_head instead of 6x.
//
// Division by G is compile-time (G=6 or G=8). The SMEM layout, warp
// specialization, Q quantization, QK MMA (s8), online softmax, and PV MMA
// (f16) are structurally identical to the original per-Q-head kernel. The only
// change is Q/output indexing: each row derives (q_head, token) from its
// packed position instead of using a single q_head for the whole tile.
// ===========================================================================

template <typename Geometry, typename Metadata>
__global__ __maxnreg__(120) void gqa_attention_prefill_i8_gqa_kernel(
    const __nv_bfloat16* __restrict__ q, const std::int8_t* __restrict__ cache_k,
    const std::int8_t* __restrict__ cache_v, const __half* __restrict__ cache_k_scale,
    const __half* __restrict__ cache_v_scale, Metadata metadata,
    const std::int32_t* __restrict__ positions, float scale, __nv_bfloat16* __restrict__ out,
    std::int32_t width) {
    constexpr int G             = Geometry::GroupSize;
    constexpr int D             = kGqaPrefillHeadDim;
    constexpr int Br            = kGqaPrefillI8Br;
    constexpr int Bc            = kGqaPrefillI8Bc;
    constexpr int DB16          = kGqaPrefillI8DB16;
    ...
    const int packed_q0 = static_cast<int>(blockIdx.x) * Br;
    const int kv_head   = static_cast<int>(blockIdx.y);
    const int tid       = static_cast<int>(threadIdx.x);
    const int warp      = tid >> 5;
    const int lane      = tid & 31;
    const int tokens    = metadata.valid_tokens(width);
    if (packed_q0 >= G * width) { return; }

    // Early exit: packed rows are all beyond valid tokens. Zero the output
    // for the G Q heads at these token positions, then return.
    if (packed_q0 >= G * tokens) {
        const int row_end = min(Br, G * width - packed_q0);
        for (int row = tid; row < row_end; row += kGqaPrefillI8Threads) {
            const int packed_r = packed_q0 + row;
            const int token_r  = packed_r / G;
            const int qh_r     = packed_r % G;
            const int q_head   = kv_head * G + qh_r;
            if (token_r < width) {
                for (int d = lane; d < D; d += 32) {
                    out[gqa_prefill_q_index<Geometry>(q_head, d, token_r)] =
    ...
    // tile_rows: number of valid rows (token < tokens) in the packed tile.
    // packed = token * G + qh, so token = packed / G. Valid iff packed < G * tokens.
    const int tile_rows     = max(0, min(Br, G * tokens - packed_q0));
    const int min_token     = packed_q0 / G;
    const int max_query_abs = tile_rows > 0 ? base_pos + (packed_q0 + tile_rows - 1) / G : -1;
    const int key_blocks    = tile_rows > 0 ? (max_query_abs / Bc + 1) : 0;
    ...
    // ---- Q quantization ----
    // Each row r in the tile: token = (packed_q0 + r) / G, qh = (packed_q0 + r) % G.
    // Rows < tile_rows are valid; rows >= tile_rows get zero Q (padding).
    for (int unit = warp; unit < Br * Groups; unit += kGqaPrefillI8Warps) {
        const int row     = unit / Groups;
        const int grp     = unit - row * Groups;
        const int d0      = grp * kGqaKvQuantGroup + lane;
        ...
        const int token_r  = packed_r / G;
        const int qh_r     = packed_r % G;
        const int q_head_r = kv_head * G + qh_r;
        float x0      = 0.0f;
        float x1      = 0.0f;
        if (row < tile_rows) {
            x0 = __bfloat162float(q[gqa_prefill_q_index<Geometry>(q_head_r, d0, token_r)]);
            x1 = __bfloat162float(q[gqa_prefill_q_index<Geometry>(q_head_r, d1, token_r)]);
        }
        float absmax    = fmaxf(fabsf(x0), fabsf(x1));
        absmax          = warp_max(absmax, FullMask);
        const float qs  = absmax > 0.0f ? absmax / 127.0f : 0.0f;
    ...
    // ---- K/V loading (same as original, indexed by kv_head) ----
    auto issue_kv_tile = [&](int tile_k0) {
        const int physical_page = block_table[tile_k0 >> kPagedKVPageShift];
        for (int key_l = tid; key_l < Bc; key_l += kGqaPrefillI8Threads) {
            const int key = tile_k0 + key_l;
...
#pragma unroll 1
    ...
    // Q scale cache (producer warps only, groups 0/1).
    float q_scale_r0[Groups - 2];
    float q_scale_r1[Groups - 2];
    if (warp < ProducerWarps) {
        const int scale_row0 = warp * 16 + gid;
...
#pragma unroll
            ...
            // Per-row causal mask: each row's query position is base_pos + token,
            // where token = (packed_q0 + row) / G (compile-time division).
            const int row0             = row_base + gid;
            const int row1             = row0 + 8;
            const int token0           = (packed_q0 + row0) / G;
            const int token1           = (packed_q0 + row1) / G;
            const int qabs0            = row0 < tile_rows ? base_pos + token0 : -1;
            const int qabs1            = row1 < tile_rows ? base_pos + token1 : -1;
            const bool full_score_tile = (tile_rows == Br) && (k0 + Bc - 1 <= base_pos + min_token);
            float bm0                  = -CUDART_INF_F;
            float bm1                  = -CUDART_INF_F;
#pragma unroll
            for (int nt = 0; nt < QKNt; ++nt) {
...
#pragma unroll
    for (int n = 0; n < PVNtPerWarp; ++n) {
        const int d0 = (d_slice * PVNtPerWarp + n) * 8 + 2 * lid;
        // Row 0: scatter to (q_head, token) derived from packed position.
        {
            const int packed_r = packed_q0 + row0;
            const int token_r  = packed_r / G;
            const int qh_r     = packed_r % G;
            if (row0 < tile_rows) {
                const int q_head = kv_head * G + qh_r;
                *reinterpret_cast<unsigned*>(
                    &out[gqa_prefill_q_index<Geometry>(q_head, d0, token_r)]) =
                    pack_bf16x2(acc[n][0] * inv_l0, acc[n][1] * inv_l0);
            } else if (token_r < width) {
                const int q_head = kv_head * G + qh_r;
                *reinterpret_cast<unsigned*>(
                    &out[gqa_prefill_q_index<Geometry>(q_head, d0, token_r)]) = 0u;
            }
        }
        // Row 1
        {
            const int packed_r = packed_q0 + row1;
            const int token_r  = packed_r / G;
            const int qh_r     = packed_r % G;
            if (row1 < tile_rows) {
                const int q_head = kv_head * G + qh_r;
